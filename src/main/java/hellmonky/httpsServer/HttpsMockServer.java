...
public class HttpsMockServer extends HttpsMockBase {
    ...
    public static byte[] readBytes(int length) throws Exception {
        byte[] undecrpty = hellmonky.httpsServer.SocketUtils.readBytes(in, length);
    ...
    public static void writeBytes(byte[] data) throws Exception {
        ...
        hellmonky.httpsServer.SocketUtils.writeBytes(out, encrpted, encrpted.length);
    }
    ...
    private static void doHttpsShakeHands(Socket s) throws Exception {
        ...
        int length = in.readInt();
        in.skipBytes(4);
        byte[] clientSupportHash = hellmonky.httpsServer.SocketUtils.readBytes(in, length);
        ...
        hellmonky.httpsServer.SocketUtils.writeBytes(out, certificateBytes, certificateBytes.length);

        System.out.println("获取客户端通过公钥加密后的随机数");
        int secureByteLength = in.readInt();
        byte[] secureBytes = hellmonky.httpsServer.SocketUtils.readBytes(in, secureByteLength);
        ...
        byte[] data = hellmonky.httpsServer.SocketUtils.readBytes(in, skip);
        ...
        byte[] clientHashBytes = hellmonky.httpsServer.SocketUtils.readBytes(in, hashSkip);
        ...
        hellmonky.httpsServer.SocketUtils.writeBytes(out, serverMessage, serverMessage.length);
        ...
        hellmonky.httpsServer.SocketUtils.writeBytes(out, serverMessageHash, serverMessageHash.length);
