package hellmonky.testPackage.httpsServer;
...
public class HttpsMockServer extends HttpsMockBase {
    ...
    public static byte[] readBytes(int length) throws Exception {
        byte[] undecrpty = hellmonky.testPackage.httpsServer.SocketUtils.readBytes(in, length);
    ...
    public static void writeBytes(byte[] data) throws Exception {
        ...
        hellmonky.testPackage.httpsServer.SocketUtils.writeBytes(out, encrpted, encrpted.length);
    }
    ...
    private static void doHttpsShakeHands(Socket s) throws Exception {
        ...
        int length = in.readInt();
        in.skipBytes(4);
        byte[] clientSupportHash = hellmonky.testPackage.httpsServer.SocketUtils.readBytes(in, length);
        ...
        hellmonky.testPackage.httpsServer.SocketUtils.writeBytes(out, certificateBytes, certificateBytes.length);

        System.out.println("获取客户端通过公钥加密后的随机数");
        int secureByteLength = in.readInt();
        byte[] secureBytes = hellmonky.testPackage.httpsServer.SocketUtils.readBytes(in, secureByteLength);
        ...
        byte[] data = hellmonky.testPackage.httpsServer.SocketUtils.readBytes(in, skip);
        ...
        byte[] clientHashBytes = hellmonky.testPackage.httpsServer.SocketUtils.readBytes(in, hashSkip);
        ...
        hellmonky.testPackage.httpsServer.SocketUtils.writeBytes(out, serverMessage, serverMessage.length);
        ...
        hellmonky.testPackage.httpsServer.SocketUtils.writeBytes(out, serverMessageHash, serverMessageHash.length);
